...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        if (!ctype_alnum(str_replace(array("@", "-", "_"), "", $username))) {
            $username_err = "používateľské meno môže obsahovať iba písmená, čísla a znaky: '@', '_', or '-'.";
        } else {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    if (mysqli_stmt_num_rows($stmt) == 1) {
                        $username_err = "Používateľské meno ktoré ste zadali je už použité";
                    }
                } else {
                    echo "<script>" . "alert('Niečo sapokazilo. Prosím skúste neskôr.')" . "</script>";
                }
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($stmt = mysqli_prepare($link, $sql)) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    if (mysqli_stmt_num_rows($stmt) == 1) {
                        $email_err = "Tento mail je už použitý.";
                    }
                } else {
                    echo "<script>" . "alert('Niečo sapokazilo. Prosím skúste neskôr.');" . "</script>";
                }
    ...
    if (empty(trim($_POST["password"]))) {
        ...
        if (strlen($password) < 8) {
            $password_err = "Heslo musí obsahovať aspoň 8 znakov";
        }
    }
            ...
            <div class="form-wrap border rounded p-4">
                ...
                <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control" name="email" id="email" value="<?= $email; ?>" placeholder="Enter Email">
                        <small class="text-danger"><?= $email_err; ?></small>
                    </div>
                    <div class="mb-2">
                        <label for="password" class="form-label"></label>
                        <input type="password" class="form-control" name="password" id="password" value="<?= $password; ?>" placeholder="Enter Password">
